...
echo "listen_addresses = '0.0.0.0'" | tee -a /etc/postgresql/$database_version/main/postgresql.conf >/dev/null
sed -i /etc/postgresql/$database_version/main/postgresql.conf -e '/^#/!s/max_connections = 100/max_connections = 3200/g'
sed -i /etc/postgresql/$database_version/main/postgresql.conf -e '/^#/!s/shared_buffers = 128MB/shared_buffers = 4GB/g'
sysctl kernel.shmmax=2147483648
echo "kernel.shmmax=2147483648" | tee -a /etc/sysctl.conf >/dev/null
#add 0.0.0.0
echo "host    all             all              0.0.0.0/0              md5" | tee -a /etc/postgresql/$database_version/main/pg_hba.conf >/dev/null
echo "host    all             all              ::/0                   md5" | tee -a /etc/postgresql/$database_version/main/pg_hba.conf >/dev/null
